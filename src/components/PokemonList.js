...
const PokemonList = () => {
  ...
  const filteredPokemonName = useMemo(
    () => filterDataByName(pokemon, filter),
    [filter, pokemon]
  );
  const sortedPokemon = useMemo(
    () => sortData(filteredPokemonName, sort),
    [sort, filteredPokemonName]
  );
              ...
              <Link to={`/pokemon/${poke.name}`}>
                <Card
                  ...
                  style={{
                    ...
                    background:
                      "-webkit-linear-gradient(45deg, rgba(213, 15, 61, 0.6), rgba(13, 17, 198, 0.69) 100%)",
                  }}
                  cover={
